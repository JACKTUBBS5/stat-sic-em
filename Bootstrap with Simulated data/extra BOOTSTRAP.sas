...
%let theta=10;           /* Right endpoint      */
%let N = 40;             /* size of each sample */
%let NumSamples = 300;   /* number of samples   */  
...
var x;
   output out=Bootdist (drop=_freq_ _type_) mean=mean_x max=max_x min=min_x median=med_x;
run;


data OutStatsUni; set Bootdist; 
stat1 = 2*mean_x; stat2=max_x; stat3=min_x + max_x; stat4=2*med_x; run;
...
proc means data=OutStatsUni mean std min max; var stat1 stat2 stat3 stat4; 
