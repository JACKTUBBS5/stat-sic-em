...
```{r}
library("glmnet")
x = data.matrix(red [,c('fix_acidity', 'vol_acidity', 'citric_acid', 'sugar', 'chlorides', 'free_sulfur', 'total_sulfur', 'density', 'pH', 'sulphates', 'alcohol')])
y = red$quality
...
```{r}
library("car")
lassomod <- lm(quality ~ vol_acidity + citric_acid + sugar + chlorides + free_sulfur + total_sulfur + pH + sulphates + alcohol, data = red)
summary(lassomod)
...
```{r}
library("car")
ridgemod <- lm(quality ~ vol_acidity + citric_acid + chlorides + density + pH + sulphates + alcohol, data = red)
summary(ridgemod)
...
```{r}
library(AICcmodavg)
models <- list(step, lassomod, ridgemod)
mod.names <- c('Stepwise', 'Lasso', 'Ridge')
