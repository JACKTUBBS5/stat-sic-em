...
title2 "Finding Principal Components";

proc princomp data = validation_set out = PCmeasures standard
plots = patternprofile; 
var vol_acidity  chlorides free_sulfur total_sulfur 
    pH sulphates alcohol; 
    run;

proc logistic desc data = PCmeasures plots = (effect roc); 
model high_quality = Prin1 - Prin7/link =logit; 
run; 

proc logistic desc data = PCmeasures plots = (effect roc); 
model high_quality = Prin1  Prin2/link =logit; 
run; 
