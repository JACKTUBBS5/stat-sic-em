...
##Changing the bandwidth of the KDE function

# The Old Faithful geyser data - waiting time

Bandwidth = 1
```{r}
fhat <- bkde(x=waiting, bandwidth=1)
plot (fhat, xlab="x", ylab="Density function")
```

Bandwidth = 5
```{r}
fhat <- bkde(x=waiting, bandwidth=5)
plot (fhat, xlab="x", ylab="Density function")
```

Bandwidth = 10
```{r}
fhat <- bkde(x=waiting, bandwidth=10)
plot (fhat, xlab="x", ylab="Density function")
```

As you increase the bandwidth, the KDE line smooths out and there is less noise. Closer to 0 keeps the noise.